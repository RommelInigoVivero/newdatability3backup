...
<head>
    ...
    <style>
        ...
        .status-column {
            width: 120px; /* You can adjust this width based on your preference */
...
<nav class="navbar navbar-expand-lg navbar-dark">
    <div class="container">
        <a class="navbar-brand" href={{route('HEAD.dashboard')}}>HEAD Portal</a>
...
<div class="container">
    ...
    <div class="d-flex justify-content-end mb-3">
        <button class="btn btn-primary"onclick="window.location='{{ route('create-account') }}'">Add Account</button>
    </div>
        ...
        <div class="modal-dialog">
            <div class="modal-content" style="background-color: #333; color: #fff;">
                ...
                <div class="modal-body" style="background-color: #444;">
                    <form method="POST" action="{{ route('HEAD.changeAdminPassword') }}">
                        @csrf
...
// Confirm the action from the modal
$('#confirmAction').click(function() {
    let url = (actionToTake === 'activate') ? '/head/admin/activate/' + adminIdToChange : '/head/admin/suspend/' + adminIdToChange;
    
    // Show spinner while making the request (optional)
    let button = $('#' + actionToTake + '-btn-' + adminIdToChange);
    ...
    $.ajax({
        ...
        success: function(response) {
            if (response.status === 'ACTIVE' || response.status === 'SUSPENDED') {
                ...
                updateButtons(adminIdToChange, response.status);
...
$('#PdaoconfirmAction').click(function() {
    let url = (actionToTakePdao === 'activate') ? '/head/user/activate/' + userIdToChange : 'head/user/suspend/' + userIdToChange;
